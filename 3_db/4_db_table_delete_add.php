...
if (isset($_GET["deleteUser"])) {
    if ($_GET["deleteUser"] == 0) {
        ...
        echo "<h4>Udało się usunąć rekord o id = $_GET[deleteUser]</h4>";
    }
}

if (isset($_GET["addUser"])){
    if ($_GET["addUser"] == 0){
        echo "<h4>Nie udało się dodać rakordu!</h4>";
    }else{
...
<table>
    ...
    $sql = "SELECT u.id userId, u.firstName, u.lastName, u.birthday, c.city, s.state, co.countries FROM users u JOIN cities c ON u.city_id=c.id JOIN state s ON c.state_id=s.id JOIN country co ON s.country_id=co.id;";
    $result = $conn->query($sql);
    //    echo $result->num_rows;

    if ($result->num_rows == 0) {
        echo "<tr><td colspan='7'>Brak rekordów do wyświetlenia</td></tr>";
    } else {
